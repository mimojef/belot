...
function renderPanelDealtCard(
  ...
  const cardWidth = mobileBottomSeat
    ? BOTTOM_HAND_MOBILE_CARD_WIDTH * bottomFanScale
    : PANEL_CARD_WIDTH
  const cardHeight = mobileBottomSeat
    ? BOTTOM_HAND_MOBILE_CARD_HEIGHT * bottomFanScale
    : PANEL_CARD_HEIGHT
  return `
    <div style="
      position:absolute;
...
function renderDealtCardFanInPanel(
  ...
  if (
    ...
    const previousCardElements = previousCards.map((card, i) => {
      const fan = getFanOffset(i, previousCards.length, compactFan, mobileBottomSeat, panelScale)
      const animStyle = `animation: belot-panel-card-hide-at-reveal 1ms linear ${animDelay}ms both;`
      return renderPanelDealtCard(card, i, fan.x, fan.y, fan.rotate, animStyle, mobileBottomSeat, panelScale)
    }).join('')
    const finalCardElements = cards.map((card, i) => {
      const fan = getFanOffset(i, count, compactFan, mobileBottomSeat, panelScale)
  ...
  const cardElements = Array.from({ length: count }, (_, i) => {
    const fanTo = getFanOffset(i, count, compactFan, mobileBottomSeat, panelScale)
    ...
    if (animDelay !== null) {
      if (isLocalSeat && card !== null) {
        ...
        if (prevIdx === undefined) {
          ...
          const fanFrom = getFanOffset(prevIdx, previousCards.length, compactFan, mobileBottomSeat, panelScale)
          animStyle = `
        ...
        if (i >= dealtHands.animStartIndex) {
          ...
          const fanFrom = getFanOffset(i, dealtHands.animStartIndex, compactFan, mobileBottomSeat, panelScale)
          animStyle = `
            --px-from:${fanFrom.x}px; --py-from:${fanFrom.y}px; --pr-from:${fanFrom.rotate}deg;
            --px-to:${fanTo.x}px; --py-to:${fanTo.y}px; --pr-to:${fanTo.rotate}deg;
            animation: belot-panel-card-reposition ${PANEL_CARD_REPOSITION_MS}ms ${PANEL_CARD_REPOSITION_EASING} ${animDelay}ms both;
    ...
    return renderPanelDealtCard(card, i, fanTo.x, fanTo.y, fanTo.rotate, animStyle, mobileBottomSeat, panelScale)
  }).join('')
