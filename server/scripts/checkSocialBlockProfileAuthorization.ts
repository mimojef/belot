...
async function main(): Promise<void> {
  ...
  await check('[4] POST vip-dm/start concurrent same/reverse directions and friend-accept race converge canonically', async () => {
    ...
    try {
      ...
      assert(raceRows.length === 1, `race left duplicate rows=${JSON.stringify(raceRows)}`)
      assert(raceRows[0]!.kind === 'friend' && raceRows[0]!.status === 'accepted', `race final row not accepted friend: ${JSON.stringify(raceRows)}`)
      ...
      assert(startAfterFriend.body.friendshipId === raceRows[0]!.friendship_id, 'start after accepted friend did not return canonical friend row')
      const raceRowsAfter = getConversationRows(databasePath, e.profileId, f.profileId)
      assert(raceRowsAfter.length === 1 && raceRowsAfter[0]!.kind === 'friend', `start after accepted friend created duplicate: ${JSON.stringify(raceRowsAfter)}`)
    } finally {
      await stopServer(server)
      await isolated.cleanup()
    }
