...
const FEATURE_GAP_AUDIT: FeatureGapEntry[] = [
  ...
  { field: 'played cards so far (пълна история извън текущия trick)', status: 'missing', note: 'Per-action visibleBeforeAction в recorder-а НЕ съдържа completed tricks — но самият deal record (TrainingDealRecord.tricks) ГИ съдържа на deal-ниво. Dataset builder-ът може да ги join-не по trickIndex БЕЗ промяна на recorder writer-а (Priority 3).' },
  { field: 'taken tricks so far (брой/точки по отбор)', status: 'missing', note: 'Същото като по-горе — derivable от deal.tricks join в dataset builder-а, не е нужна recorder writer промяна (Priority 3).' },
  { field: 'score context (running точки в текущата ръка)', status: 'missing', note: 'scoreBeforeDeal е само ПРЕДИ тази ръка (match carry-over) — точките в ТЕКУЩАТА ръка не се пазят per-action, но са derivable от deal.tricks[].points join (Priority 3).' },
  { field: 'declarations context (обявени терца/50/100/каре/белот)', status: 'missing', note: 'TrainingDealRecord НЯМА declarations поле изобщо — нито на deal, нито на action ниво. Изисква recorder writer промяна, за да се захване (Priority 4).' },
  { field: 'capot risk/context', status: 'derivable', note: 'След Priority 3 join (taken tricks so far по отбор), "0 трика за отбор X след N изиграни трика" е тривиално производно.' },
  ...
  { field: 'own trump count', status: 'derivable', note: 'Брой карти в ownHand, за които isTrump()===true при текущия contract/trumpSuit — не варира между candidate картите в едно решение (виж методологичната бележка в recommendations).' },
  { field: 'whether candidate card can currently win the trick', status: 'derivable', note: 'Reuse на съществуващия getServerTrickWinner() (server/src/game/getServerTrickWinner.ts) с currentTrick + candidate card — точно същата чиста функция, използвана от gameplay/scoring, не е ново правило. Използвано в тази диагностика (wouldCardWinTrick).' },
  { field: 'whether trick is already partner-winning', status: 'derivable', note: 'deriveTeam(currentWinningSeat) === deriveTeam(seat), когато currentWinningSeat не е null.' },
  { field: 'whether trick is already opponent-winning', status: 'derivable', note: 'Обратното на горното — вече частично закодирано в trickLeadershipSignal interaction терми, но не и като самостоятелен non-interacted feature.' },
  { field: 'points currently in trick', status: 'derivable', note: 'sum(getServerCardPoints(...)) върху currentTrick — вече налична информация, само не се извежда като feature.' },
  { field: 'last trick context', status: 'missing', note: 'Изисква Priority 3 join от deal.tricks[trickIndex-1] — derivable от recorder архива, без recorder writer промяна.' },
  { field: 'played cards / void tracking на другите играчи (inferred от история)', status: 'unsafe_unclear', note: 'Технически derivable от played-card история (Priority 3 join) БЕЗ да разкрива скрита информация (само карти, реално показани на масата) — но изисква внимателен design review, за да не изтече информация от бъдещи actions в СЪЩИЯ trickIndex. Означено unsafe/unclear до design review, не Priority-класирано директно.' },
]
