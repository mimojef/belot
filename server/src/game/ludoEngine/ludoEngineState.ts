...
import { LUDO_CANONICAL_TURN_ORDER } from './ludoGeometryConstants.js'
import type { LudoColor, LudoGamePiece, LudoGameState, LudoPieceSlot } from './ludoEngineTypes.js'
...
// Production multiplayer state: unlike the historical visual-prototype
// factory above, every pawn starts in its canonical home slot and only the
// colors assigned to real room participants take turns.
export function createLudoAuthoritativeInitialState(turnOrder: readonly LudoColor[]): LudoGameState {
  if (turnOrder.length !== 2 && turnOrder.length !== 4) {
    throw new Error(`Ludo match requires 2 or 4 colors, got ${turnOrder.length}`)
  }
  return {
    turnOrder: [...turnOrder],
    activeColor: turnOrder[0]!,
    turnPhase: 'waiting_for_roll',
    diceValue: null,
    legalMoves: [],
    pieces: turnOrder.flatMap((color) => ([0, 1, 2, 3] as const).map((slot) => homePiece(color, slot))),
    status: 'in_progress',
    winnerColor: null,
    turnVersion: 0,
    pendingExtraRoll: false,
